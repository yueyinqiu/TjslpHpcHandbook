...
uv 现在已成为现代化 Python 开发的事实标准，统一了 Python 版本管理、包管理、项目管理等功能。可能我们更熟悉的是 Anaconda ，但如果没有什么非常复杂的 C/C++ 依赖需要处理，建议切换到 uv 。
...
# uv 自更新镜像； Python 下载镜像。如果不想使用，可跳过。
...
# PyPI 镜像。这会在项目的 pyproject.toml 中自动添加相应配置。如果不希望该行为，可以在 `~/.config/uv/uv.toml` 配置镜像，而不使用环境变量，见 https://mirrors.tuna.tsinghua.edu.cn/help/pypi/
...
使用 uv 时，一般不采用 `activate` 激活环境（虽然它也支持），而是直接在命令前添加 `uv run` ：

```sh
uv run main.py    # 或者 uv run python main.py
```
...
如果习惯了手动管理 Python 环境，现在切换到 uv 可能会有一丝不安。不过，这里没有魔法，事实上环境就位于 `.venv` 中：

```sh
ls .venv
```

然而，我们还要进一步强调，环境并不重要。让我们删掉虚拟环境：
...
刚刚的 `variable-declaration-checker` 将会被移除。因为 `pyproject.toml` 中没有它。

由于 `pyproject.toml` 是可以由 git 等版本管理工具追踪的，与它的完全同步，意味着整个环境都是可以进行版本管理、可以稳定复现的（这同时也依赖于 `uv.lock` ）。这会非常有帮助。

## 第七步 安装 PyTorch

然而不得不承认，这样的管理对于 PyTorch 等依赖硬件环境的包，事情会变得复杂。

### 直接从默认源安装 PyTorch

当然，这其实不是 uv 的问题，而是 PyPI 本身设计时，没有考虑如何区分不同 CUDA 版本。
...
当然，如果仅仅是为了把 PyTorch 安装上，仍然可以使用：
...
这意味着，具体 `torch` 应该从什么源下载并没有被记录。当 uv 认为当前 `torch` 版本不合适，需要重新下载时，它可能从默认 PyPI 源下载。

### 在 `pyproject.toml` 指定源

更好的方式是指定下载源。建议直接手动编辑 `pyproject.toml` ，例如：

```toml {hl_lines=[10, "17-27"]}
[project]
